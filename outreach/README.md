...
3. **Enriches** each company: **Apollo** (free) finds real headcount, website, company LinkedIn, and the right decision maker (founder/CEO for startups, CRO/VP Sales for big companies); **Hunter.io** finds that person's **email** from their name + domain
...
### 4. Get your API keys
- **Apollo**: apollo.io → Settings → Integrations → API → create key.
  Used only to *find* the decision maker (name/title/LinkedIn) and the company
  domain — that's **free**, no paid plan or email credits needed.
- **Hunter.io**: hunter.io → API → copy key. Finds the decision maker's email
  from their name + company domain. Free plan ≈ 25–50 lookups/month (1 per company).
...
|---|---|---|
| Header Auth | `Apollo` | Name: `X-Api-Key`, Value: your Apollo key |
...
2. In each workflow, open the nodes that show a ⚠️ and:
   - **Apollo nodes** (Find Company, Find People) → select the `Apollo` credential
   - **Hunter: Find Email** node → select the `Hunter` credential
...
| Find company | Apollo (free) | Company match → headcount / website / LinkedIn / domain |
| Find person | Apollo (free) | Decision-maker search by title (no email credits used) |
| Find email | Hunter.io | Email from name + domain; only accepts score ≥ 70 or verified-valid |
| Targeting | n8n Code | ≤500 employees → founder/CEO first; >500 → CRO/VP Sales/Growth first |
...
| Send | Gmail | Draft or auto-send with resume attached, no n8n attribution footer |
...
- **Email confidence bar**: `Absorb Email` node — `score >= 70` threshold; raise it to be stricter, lower to get more (riskier) emails
...
- **Decision-maker titles**: edit the `person_titles` list in *Apollo: Find People*
...
- **Hunter credits**: each company = 1 Hunter lookup. Free tier ≈ 25–50/month, so
  keep `Limit Per Run` around 2/day. Apollo's people/company search stays free —
  no Apollo paid plan needed.
